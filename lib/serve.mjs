...
// and returns the cache as it stands. A POST to /refresh starts one whatever the data's age.
...
export async function startServer({ adapter, data: dataFile, interval = 60, poll = 10, port = 4173, timeout = 120 }) {
  ...
  let lastFingerprint;
  let inFlight = null;
  // Set by /refresh during a run, which may have read the tracker before the change the caller
  // wants to see. Cleared by close(), so a killed run starts nothing after it.
  let again = false;
  ...
  const server = createServer((req, res) => {
    ...
    // Sec-Fetch-Site ignores the port, so a page on another local port is same-site. A browser
    // sends Origin with every POST, which tells that page apart.
